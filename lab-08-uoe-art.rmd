...
knitr::include_graphics("/home/tmchuynh/Documents/Data Science/lab-08-tmchuynh/img/selectorgadget.png")
```
...
```{r iteminfo-h3a, fig.margin = TRUE, echo = FALSE, eval = TRUE}
...
# Create sequence of offsets
offsets <- seq(from = 0, to = 2960, by = 10)

# Construct URLs
urls <- glue("{root}{offsets}")
```
...
```{r eval = TRUE}
# Load the scrape_page function from previous script
source("scripts/02-scrape-page-function.R")

# Scrape all pages and create uoe_art data frame
...
```{r eval = TRUE}
# Write the data frame to a CSV file in the data folder
...
**Explanation of warnings:** The warnings occur because some entries don't have parentheses (so `separate()` creates NA values) and some date strings don't contain 4-digit years (so `as.numeric()` creates NA values). This is expected and acceptable since not all art pieces have date information, and our goal is simply to extract year information where it's readily available.
...
Based on the summary, we can see the number of missing values for artist and year information in the output above.

11. Make a histogram of years. Use a reasonable binwidth. Do you see anything out of the ordinary?

```{r histogram-years, eval=TRUE}
uoe_art %>%
  filter(!is.na(year)) %>%
  ggplot(aes(x = year)) +
  geom_histogram(binwidth = 10, fill = "steelblue", alpha = 0.7) +
  labs(
    title = "Distribution of Art Piece Years",
    x = "Year",
    y = "Count"
  ) +
  theme_minimal()
...
There appears to be an outlier with a very early year (possibly around year 2 or similar), which seems historically implausible for this collection.
...
# Find the piece with the unusual year
uoe_art %>%
  filter(!is.na(year)) %>%
  arrange(year) %>%
  slice(1) %>%
  select(title, date, year, link)
```

```{r correct-outlier, eval=TRUE}
# Correct the outlier year (assuming the piece with year 2 should be from a different century)
uoe_art <- uoe_art %>%
  mutate(
    year = case_when(
      year == 2 ~ 2002, # Adjust based on actual investigation of the artwork
      TRUE ~ year
    )
  )
...
uoe_art %>%
  ...
  geom_histogram(binwidth = 10, fill = "steelblue", alpha = 0.7) +
  labs(
    title = "Distribution of Art Piece Years (Corrected)",
    x = "Year",
    y = "Count"
  ) +
  theme_minimal()
...
The error likely occurred because our regex pattern `\\d{4}` captured any 4 consecutive digits, which might have been part of a longer number or date format that didn't represent the actual year.
...
The most commonly featured artist can be identified from the output above. They might be well-represented because they could be a faculty member, alumnus, or have a special relationship with the Edinburgh College of Art.
...
```{r count-child-titles, eval=TRUE}
# Count titles containing "child" (case-insensitive)
